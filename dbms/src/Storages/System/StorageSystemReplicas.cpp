...
StorageSystemReplicas::StorageSystemReplicas(const std::string & name_)
    ...
    , columns{
        ...
        { "last_queue_update_attempt",            std::make_shared<DataTypeDateTime>() },
        { "last_successful_queue_update_attempt", std::make_shared<DataTypeDateTime>() },
    ...
    const unsigned threads)
{
    ...
    ColumnWithTypeAndName col_last_queue_update_attempt{std::make_shared<ColumnUInt32>(), std::make_shared<DataTypeDateTime>(), "last_queue_update_attempt"};
    ColumnWithTypeAndName col_last_successful_queue_update_attempt{std::make_shared<ColumnUInt32>(), std::make_shared<DataTypeDateTime>(), "last_successful_queue_update_attempt"};
    ...
    for (size_t i = 0, size = col_database.column->size(); i < size; ++i)
    {
        ...
        col_last_queue_update_attempt.column->insert(UInt64(status.last_queue_update_attempt));
        col_last_successful_queue_update_attempt.column->insert(UInt64(status.last_successful_queue_update_attempt));
    ...
    Block block{
        ...
        col_last_queue_update_attempt,
        col_last_successful_queue_update_attempt,
        col_total_replicas,
        col_active_replicas,
    };
